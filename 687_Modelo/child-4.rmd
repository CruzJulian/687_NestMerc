```{r}
completo_analisis %$% 
  extract(nombre_cons_es, constructo == "e_WOM") -> nombre_del_constructo
...
completo_analisis %$% 
  extract2(cors, which(constructo == "e_WOM")) %>% 
  corrplot.mixed(
    lower = "number", 
    upper = "color", 
...
```{r}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "e_WOM")) %$% sedim_caption -> sedim_cap

```

```{r, fig.show = "asis", fig.cap = sedim_cap, fig.height= 2.5, fig.width = 6}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "e_WOM")) %$% sedim_plot 
...
```{r}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "e_WOM")) %$% var_caption -> var_caption

```

```{r, fig.show = "asis", fig.cap = var_caption, fig.height= 4, fig.width = 6}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "e_WOM")) %$% var_plot 
...
completo_analisis %$% 
  extract2(alphas, which(constructo == "e_WOM")) -> reability


completo_analisis %$% 
  extract2(shapiro, which(constructo == "e_WOM")) -> shapiro_wilks

completo_analisis %$% 
  extract2(lillie, which(constructo == "e_WOM")) -> lilliefors
