```{r}
completo_analisis %$% 
  extract(nombre_cons_es, constructo == "BI") -> nombre_del_constructo
...
completo_analisis %$% 
  extract2(cors, which(constructo == "BI")) %>% 
  corrplot.mixed(
    lower = "number", 
    upper = "color", 
...
```{r}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "BI")) %$% sedim_caption -> sedim_cap

```

```{r, fig.show = "asis", fig.cap = sedim_cap, fig.height= 2.5, fig.width = 6}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "BI")) %$% sedim_plot 
...
```{r}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "BI")) %$% var_caption -> var_caption

```

```{r, fig.show = "asis", fig.cap = var_caption, fig.height= 4, fig.width = 6}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "BI")) %$% var_plot 
...
completo_analisis %$% 
  extract2(alphas, which(constructo == "BI")) -> reability


completo_analisis %$% 
  extract2(shapiro, which(constructo == "BI")) -> shapiro_wilks

completo_analisis %$% 
  extract2(lillie, which(constructo == "BI")) -> lilliefors
