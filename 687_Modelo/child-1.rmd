```{r}
completo_analisis %$% 
  extract(nombre_cons_es, constructo == "SI") -> nombre_del_constructo
...
completo_analisis %$% 
  extract2(cors, which(constructo == "SI")) %>% 
  corrplot.mixed(
    lower = "number", 
    upper = "color", 
...
```{r}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "SI")) %$% sedim_caption -> sedim_cap

```

```{r, fig.show = "asis", fig.cap = sedim_cap, fig.height= 2.5, fig.width = 6}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "SI")) %$% sedim_plot 
...
```{r}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "SI")) %$% var_caption -> var_caption

```

```{r, fig.show = "asis", fig.cap = var_caption, fig.height= 4, fig.width = 6}
completo_analisis %$% 
  extract2(analisis_multi, which(constructo == "SI")) %$% var_plot 
...
completo_analisis %$% 
  extract2(alphas, which(constructo == "SI")) -> reability


completo_analisis %$% 
  extract2(shapiro, which(constructo == "SI")) -> shapiro_wilks

completo_analisis %$% 
  extract2(lillie, which(constructo == "SI")) -> lilliefors
